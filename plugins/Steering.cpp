...
namespace{
	...
	/*
	class MapObject{
		std::vector< pair<Vec3,Vec3> > edges;
		public:
			MapObject(){}
			
			~MapObject(){}
			
			void addEdge(Vec3& p1, Vec3& p2){
				edges.push_back(pair(p1,p2));
			}
			
			bool checkCollision(const Vec3& ray, Vec3& normal){
	...
	class CollisionDetector{
		static void getCollision(Vec3& position, Vec3& rayVector, Collision& collision){
			
		}
	}
	
	class SteeringObstacleAvoidance{
		public:
			static const float avoidDistance;
			static const float lookahead
			static void getSteering(const Kinematic& target, Kinematic& character, SteeringOutput& steering){
				Vec3 rayVector = character.velocity.normalize();
				rayVector *= lookahead;
				Collision collision;
				CollisionDetector::getCollision(character.position, rayVector, collision);
				if(!collision.collisionDetected())
					return;
				Kinematic seekTarget = collision.position + collision.normal * avoidDistance;
				SteeringSeek(seekTarget, character, steering);
				
				character.setBehaviourName("STEERINGOBSTACLEAVOIDANCE");
			}
	};
